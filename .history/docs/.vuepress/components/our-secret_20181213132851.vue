<template>
    <div class="our-secret-wrapper" v-show="showMask">
        <div class="input-secret-box">
            <h1>😘Love Story😊</h1>
            <label>请输入暗号：<input type="text" autocomplete="off" v-model="ourSecret" @change="submitSecret"></label>
...
export default {
    ...
    methods: {
        submitSecret() {
            ...
            // this.showMask = window.btoa(this.ourSecret) !== "Z3RsbQ==";
            this.showMask = this.ourSecret !== "\u0067\u0074\u006c\u006d";
            // this.showMask = this.ourSecret !== unescape("\u0067\u0074\u006c\u006d".replace(/\\/g, "%"));
            // this.showMask = this.ourSecret !== window.atob('gtlm');
            // this.hintText = this.ourSecret !== 'gtlm' ? '暗号不对哦😣,快询问管理员！': '';
            // this.hintText = window.btoa(this.ourSecret) !== "Z3RsbQ==" ? '暗号不对哦😣,快询问管理员！': '';
    ...
    .our-secret-wrapper{
        ...
        .input-secret-box{
            ...
            h1{
                ...
                font-size: 2rem;
                color: #be7272;
                background-image: -webkit-gradient(linear, 0 0, 0 bottom, from(rgba(165, 249, 255, 1)), to(rgba(41, 208, 254, 1)));
