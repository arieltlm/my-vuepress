...
export default {
    ...
    mounted() {
        debugger;
        if (unescape(sessionStorage.getItem('oursecret').replace(/\\/g, "%")) === "\u0067\u0074\u006c\u006d") {
            debugger;
            this.showMask = false;
        }
    },
    methods: {
        submitSecret() {
            ...
            let res = [];
            for ( var i=0; i<this.ourSecret.length; i++ ) {
                res[i] = ( "00" + this.ourSecret.charCodeAt(i).toString(16) ).slice(-4);
            }
            sessionStorage.setItem('oursecret',"\\u" + res.join("\\u"));
            this.ourSecret = '';
        }
    }
}
